rule("compiler_setup")
    on_config(function(target)
        if target:has_tool("cxx", "cl") then
            ...
            target:add("cxflags", "/we4715", {tools = "cl"}) -- not all control paths return a value (warning as error)
            target:add("cxflags", "/we4716", {tools = "cl"}) -- must return a value (warning as error)
